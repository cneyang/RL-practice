...
class GameState:
    ...
    def getLegalActions(self, agentIndex=0):
        ...
        if self.isWin() or self.isLose():
            return []
        #print("pacman.getLegalAction.agentIndex", agentIndex)
    ...
    def getGhostState(self, agentIndex):
        #print("pacman.py.getGhostState", agentIndex, self.getNumAgents())
...
TIME_PENALTY = 1  # Number of points lost each round (default=1)
...
def readCommand(argv):
    ...
    parser.add_option('-n', '--numTests', dest='numGames', type='int',
                      help=default('the number of Test to play'), metavar='GAMES', default=1)
    ...
    parser.add_option('-a', '--agentArgs', dest='agentArgs',
                      ...
                      #default='double=False,multistep=False')
    ...
    parser.add_option('--multistep', dest='multistep', type='int', default=0)
    ...
    #print("pacman options.pacman", options.pacman)

    if(options.pacman == 'KeyboardAgent'):
        pacman = pacmanType(**agentOpts)
    else:
        if options.numTraining > 0:
            args['numTraining'] = options.numTraining
        agentOpts['numTraining'] = options.numTraining
        agentOpts['double'] = options.double
        if options.multistep > 1:
            agentOpts['multistep'] = True
            agentOpts['n_steps'] = options.multistep
        else:
            agentOpts['multistep'] = False
            agentOpts['n_steps'] = options.multistep
        agentOpts['numTesting'] = options.numGames
        if options.numTraining ==0:
            agentOpts['trained_model'] = True
        else:
            agentOpts['trained_model'] = False
    ...
    args['numGames'] = options.numGames + options.numTraining
...
def runGames(layout, pacman, ghosts, display, numGames, record, numTraining=0, catchExceptions=False, timeout=30):
    import __main__
    #print("pacman.runGames.display ", display)
